...
- UI validation: `new_page(url=...)` / `navigate_page(url=...)` → `take_snapshot` → interact (`click` / `fill` / `press_key`) → `wait_for(text=...)` → `take_screenshot` (optional)
...
- Skills are reusable, task-specific workflows packaged as `SKILL.md` instructions.
- If the task explicitly names a skill, use it immediately and follow its workflow.
- If the task involves page/UI design (layout, visual system, UX polish, accessibility), use the `ui-ux-pro-max` skill.
